...
setwd("C:\\Users\\kelli\\OneDrive\\Documents\\Duckweed\\adgenet\\2023\\uklight_degenotate")
...
#vcf <- read.vcfR("cohort2.vcf.gz", nrows=3000000)
#got to 2806000 failed ks, added in nrows
#vcf <- read.vcfR("cohort2.vcf.vcf")
#vcf <- read.vcfR("cohort2.vcf") #doesnt load vcf

#Ana# for BF test (filtered file) I'll import the raw .gz file that Razi sent me 
#vcf <- read.vcfR("BF_ready/filtered.F4.vcf.gz",  nrows=3000000)  #read in only 3 million variants
#variants 361311
#variants 438535
vcf <- read.vcfR("uklight_degen.vcf")
#harsh prune variants 104
#vcf <- read.vcfR("uklight_degen_ld_pruned_mis0.8_maf0.05_50_50_0.1.vcf")
...
length(aa.genlight$ind.names) #34

labels  = list(
  'ERR3957957'  = c('S. intermedia 8410',  'WW',  'Sintermedia'),
  'L.min'  = c('Lminuta7760',  'WW',  'Lminuta'),
  'SRR10958765'  = c('Lminor9441',  'WW',  'Lminor'),
  'SRR10958787'  = c('Lminor7868',  'WW',  'Lminor'),
  'SRR11472010'  = c('Spolyrhiza9504',  'WW',  'Spolyrhiza'),
  'SRR8291590'  = c('Lturionifera9434',  'WW',  'Lturionifera'),
  'SRR8291593'  = c('Lminuta9581',  'WW',  'Lminuta'),
  'SRR8291594'  = c('Lminuta9484',  'WW',  'Lminuta'),
  'SRR8291595'  = c('Lminuta7612',  'WW',  'Lminuta'),
  'SRR8291596'  = c('Lminuta6717',  'WW',  'Lminuta'),
...
pca.1$eig[1]/sum(pca.1$eig) # 43% raw, 55% harsh prune proportion of variation explained by 1st axis
pca.1$eig[2]/sum(pca.1$eig) # 24% raw, 22% harsh prune proportion of variation explained by 2nd axis
...
col <- funky(5) #Ana# adjust the number here to the total of locations you have
...
g1 <- s.class(pca.1$scores, pop(aa.genlight), xax=1, yax=2, col=transp(col,1),
                ...
                ylab="PC2 = 24%", xlab="PC1 = 41%", ppoints.cex=0.5, ppoints.col="black")
...
# save nice figs
pdf ("PCA_uklight_degen_named_harshprune.pdf", width=14, height=7)
...
title(expression("Neighbour-joining tree of "*italic(UK~duckweeds)*" "))
#save the tree using:
write.tree((NJtree),file="NJ.tree_populations_4ds_uklight_degen.tre")
...
# save the grouping
write.table(grp$grp, file="grouping_all_4ds_4clusters.txt", sep="\t", quote=F, col.names=F)
...
title(expression("Neighbour-joining tree of distance-based analysis of "*italic(B.~fruticulosa)*" "))
